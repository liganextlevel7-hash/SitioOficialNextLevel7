...
async function downloadPNG() {
  // Siempre releemos el filtro seleccionado (Jornada/Fecha/Equipo) antes de generar el PNG,
  // así nunca descarga datos de una jornada anterior que ya no está seleccionada.
  await cargarDatos();
  if (!ultimosFiltrados.length) { alert('Primero selecciona una jornada, fecha o equipo válido.'); return; }
...
async function downloadPNGv2() {
  // Mismo resguardo: releer el filtro seleccionado antes de generar este PNG también.
  await cargarDatos();
  if (!ultimosFiltrados.length) { alert('Primero selecciona una jornada, fecha o equipo válido.'); return; }
